//+------------------------------------------------------------------+
//|                   FX Lab The X10 v1.218_mainSPTPclosesAllPos.mq5 |
...
void CheckZoneLogic(double value)
{
   ...
   if(value >= 100)
   {
      ...
      if(!upperEntryDone)
      {
         // 🔴 ВРЕМЕННО ПУСТО (подключим позже)
         upperEntryDone = true;
      }
   }
   else
...
// v1.213 переделали корректность настройки для подачи сигнала при выходе из зон cci vtoroi raz
// v1.213 упростили код     
// v1.214 исправление not final!!! SIGNAL_ARROW = 0, SIGNAL_LEVEL = 1,SIGNAL_CROSS = 2,
// v1.215 LEVEL → добавляем ENTER / EXIT (состояния)  👉 работают одинаково:CROSS ✔LEVEL ✔
// v1.216 исправлена функция для использования стрелочных индикаторов
// v1.217 auto detecd buffers function added
